...
import { IconDotsVertical, IconFilterFilled, IconTableColumn, IconEdit, IconTrash, IconSearch, IconX } from '@tabler/icons-vue';
...
const applyFilter = () => {
    ...
    router.get(route(`${module}.index`), params, {
        preserveState: true,
    })
...
const sortColumn = (key) => {
    ...
    router.get(route(`${module}.index`), { sort: sort }, { preserveState: true });
}
...
const changePageOptions = () => {
    router.get(route(`${module}.index`), { limit: pageOptionValue.value }, { preserveState: true })
}

const onPageChanged = (page) => {
    router.get(route(`${module}.index`), { page: page, limit: pageOptionValue.value }, { preserveState: true })
}
...
<template>
    <!-- table container -->
    <div class="bg-white border border-skin-neutral-5 rounded-lg">
        <!-- search dan filter -->
        <div class="grid grid-cols-2">
            ...
            <div class="relative flex justify-start items-center gap-2 px-3">
                ...
                <Transition enter-active-class="transition-opacity duration-500 ease-in-out"
                    ...
                    <div v-if="openAction" v-click-away="() => openAction = false"
                        class="z-10 bg-white w-1/4 absolute top-[85%] border border-skin-neutral-5 rounded-lg p-1">
            ...
            <div class="relative flex justify-end items-center gap-2 px-3">
                <!-- search -->
                <div class="rounded-tl rounded-tr pb-4 pl-3 pt-3">
                    ...
                    <div class="flex items-center align-middle relative">
                        ...
                        <InputText id="search" @input="onSearchInput" v-model="filter.search" :placeholder="Search"
                ...
                <!-- custom filter -->
                <div class="relative">
                    <IconFilterFilled class="size-6 hover:cursor-pointer text-skin-neutral-9 hover:text-black"
                ...
                <Transition enter-active-class="transition-opacity duration-500 ease-in-out"
                    ...
                    <div v-if="openFilter" v-click-away="() => openFilter = false"
                        class="z-10 bg-white w-1/2 absolute top-[85%] border border-skin-neutral-5 rounded-lg p-2">
                ...
                <!-- toggle columns -->
                <IconTableColumn class="size-6 hover:cursor-pointer text-skin-neutral-9 hover:text-black"
                    @click="openToggleColumn = !openToggleColumn" />
                <Transition enter-active-class="transition-opacity duration-500 ease-in-out"
                    leave-active-class="transition-opacity duration-500 ease-in-out" enter-from-class="opacity-0"
                    enter-to-class="opacity-100" leave-from-class="opacity-100" leave-to-class="opacity-0">
                    <div v-if="openToggleColumn" v-click-away="() => openToggleColumn = false"
                        class="z-10 bg-white w-1/2 absolute top-[85%] border border-skin-neutral-5 rounded-lg p-4">
                            ...
                            <div class="flex flex-col gap-2 justify-between">
                                <div v-for="column in columns" :key="column.key" class="flex gap-3 items-center">
                                    <Checkbox :id="column.key" v-model="column.visible"
                                        @change="toggleColumn(column.key)" class="text-skin-neutral-12 focus:ring-0" />
        ...
        <!-- selected rows -->
        <div class="flex justify-between items-center border-t bg-skin-neutral-2 py-2 px-4 text-sm font-bold"
        ...
        <div class="flex justify-between items-center border-t bg-skin-neutral-2 py-2 px-4"
        ...
        <!-- table -->
        <div class="overflow-x-auto border-t border-skin-neutral-5" v-if="items.data.length">
            <table class="w-full table-auto text-left text-sm">
                <!-- table head -->
                <thead>
                    <tr>
                        <th class="bg-skin-neutral-2 px-4 py-2">
                        ...
                        <th v-for="column in visibleColumns" :key="column.key"
                            class="bg-skin-neutral-2 px-4 py-2 hover:cursor-pointer" @click="sortColumn(column.key)">
                            {{ column.label }}
                            <span v-if="sortKey === column.key">
                                {{ sortOrder === 'asc' ? '↑' : '↓' }}
                            </span>
                        </th>
                        <th class="bg-skin-neutral-2 px-4 py-2">Action</th>
                    </tr>
                </thead>
                <!-- end table head -->
                ...
                <tbody class="bg-white">
                    <tr class="border-t border-skin-neutral-5 hover:bg-skin-neutral-3" v-for="item in items.data"
                        :key="item.id">
                        ...
                        <td class="whitespace-nowrap px-4 py-2 font-medium">
                            ...
                            <div class="inline-block mr-2">
                                <button type="button"
                                    ...
                                    @click="$inertia.visit(route(`${module}.edit`, item.id))">
                            ...
                            <div class="inline-block mr-2">
                                <button type="button"
                                    ...
                                    @click="confirmDelete(route(`${module}.destroy`, item.id))">
                                    <IconTrash class="size-4" />
                                    Delete
                                </button>
                            </div>
        ...
        <!-- empty state -->
        <div v-if="!items.data.length" class="text-center p-4 flex flex-col gap-2">
    ...
    <!-- pagination -->
    <div class="flex justify-between items-center p-4" v-if="items.data.length">
        <span class="mt-4 text-sm">Showing {{ items.from }} to {{ items.to }} of {{ items.total }} entries</span>
        <div class="mt-4 rounded-lg text-sm grid grid-cols-2 divide-x-4 divide-slate-400 items-center justify-evenly">
            <span class="">Showing</span>
            <select v-model="pageOptionValue" @change="changePageOptions" class="border-none focus:ring-0 ">
                <option v-for="option in pageOptions" :key="option" :value="option">
                    <span class="text-sm">{{ option }}</span>
                </option>
            </select>
        </div>
        <FwbPagination :total-pages="items.last_page" v-model="currentPage" :per-page="limit" :total-items="items.total"
            v-on:page-changed="onPageChanged" :enable-first-and-last-buttons="true" class="text-sm mt-4 justify-center">
        </FwbPagination>
    </div>
    <!-- end pagination -->
